...
def ensure_hypertables_exist():
    ...
    with engine.begin() as conn:
        ...
        # Add columns if missing (for backwards compatibility)
        conn.execute(text("""
            ALTER TABLE weather_predictions ADD COLUMN IF NOT EXISTS latitude DOUBLE PRECISION;
            ALTER TABLE weather_predictions ADD COLUMN IF NOT EXISTS longitude DOUBLE PRECISION;
            ALTER TABLE weather_actuals ADD COLUMN IF NOT EXISTS latitude DOUBLE PRECISION;
            ALTER TABLE weather_actuals ADD COLUMN IF NOT EXISTS longitude DOUBLE PRECISION;
        """))
...
def write_forecasts_to_timescale(forecast_df: pd.DataFrame):
    ...
    try:
        ...
        cols = ["time", "target", "target_display", "horizon", "predicted_value", "location", "latitude", "longitude"]
        if "location" not in df_to_write.columns:
            df_to_write["location"] = LOCATION_NAME
        if "latitude" not in df_to_write.columns:
            df_to_write["latitude"] = LATITUDE
        if "longitude" not in df_to_write.columns:
            df_to_write["longitude"] = LONGITUDE
...
def write_actuals_to_timescale(df: pd.DataFrame, target_columns: list, latitude: float = LATITUDE, longitude: float = LONGITUDE, location_name: str = LOCATION_NAME):
    """
